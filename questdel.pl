...
my $old_perl_data = $old_json_object->decode($old_json_data);


delete $old_perl_data->{"04"};


my $minimal_json_data = "{}";
my $new_json_object = JSON::PP->new;
my $new_perl_data = $old_json_object->decode($minimal_json_data);

my $real_number;
for (my $number=1; $number <= 99; $number++) {
	$number = sprintf ("%02d", $number);
	if (defined ($old_perl_data->{$number})) {
		if (length ($old_perl_data->{$number}->{question}) > 0) {
			$real_number++;
			$real_number = sprintf ("%02d", $real_number);
			$new_perl_data->{$real_number} = $old_perl_data->{$number};
		}
	}
}

my $pretty_json_data = $new_json_object->pretty->sort_by(sub {$JSON::PP::a cmp $JSON::PP::b})->encode($new_perl_data);
